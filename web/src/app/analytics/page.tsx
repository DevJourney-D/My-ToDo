...
const BarChart = dynamicImport(() => import('recharts').then(mod => mod.BarChart), { 
    ssr: false,
...
const PieChart = dynamicImport(() => import('recharts').then(mod => mod.PieChart), { 
    ssr: false,
    loading: () => <div className="h-64 bg-gray-100 animate-pulse rounded"></div>
});
const Pie = dynamicImport(() => import('recharts').then(mod => mod.Pie), { ssr: false });
const Cell = dynamicImport(() => import('recharts').then(mod => mod.Cell), { ssr: false });// โครงสร้างข้อมูลจำลอง, ให้แทนที่ด้วย Type จาก API จริงของคุณ
                            ...
                            <PieChart>
                                <Pie 
                                    ...
                                    label={({ name, percent }: { name: string; percent?: number }) => (
                                      <tspan>{name} ({percent ? (percent * 100).toFixed(0) : 0}%)</tspan>
                                    )}
                                >
